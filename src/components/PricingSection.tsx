      ...
      <div className="container mx-auto px-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 max-w-5xl mx-auto">
          {plans.map((plan, index) => (
            <div
              key={index}
              className={`rounded-lg md:rounded-xl p-4 md:p-6 card-hover ${
                plan.featured
            ...
            >
              <div className="text-center mb-4 md:mb-6">
                <h3 className={`font-bold text-base md:text-lg mb-2 ${plan.featured ? "" : "text-foreground"}`}>
                  {plan.name}
                </h3>
                <p className={`text-xs md:text-sm mb-3 md:mb-4 ${plan.featured ? "text-primary-foreground/80" : "text-muted-foreground"}`}>
                  {plan.description}
                </p>
                <div className="flex items-baseline justify-center gap-1">
                  <span className={`text-xs md:text-sm ${plan.featured ? "" : "text-muted-foreground"}`}>R$</span>
              ...
              <ul className="space-y-2 md:space-y-3 mb-4 md:mb-6">
                {plan.features.map((feature, i) => (
                  <li key={i} className="flex items-center gap-2 md:gap-3">
                    <span className={`w-4 md:w-5 h-4 md:h-5 rounded-full flex items-center justify-center flex-shrink-0 ${
              ...
              <button className={`w-full py-2 md:py-3 rounded-lg font-semibold transition-colors ${
                plan.featured
